...
ggplot(eua_prices, aes(x=Date, y=Price)) + geom_line() + ggtitle("EU Emission Allowance Price") + ylab("Euro/tCO2")
```
...
Now for every year the EU counts the total number of allowances that are circulation (TNAC) on 31st of December using this formula:
...
of the 50 million initial TNAC will be moved to the reserve, i.e. roughly 37.5 million. (The term `(1-0.24)^4` comes from the years 2020-2023 in which 24% of TNAC are moved to the MSR and the term `(1-0.12)^2` from the years 2024-2025 where only 12% are moved to the MSR.)
...
### Discussion about Risks, Impact and Political Reaction
